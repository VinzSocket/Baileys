...
/** A single native flow button, already normalized, or one of the common legacy shapes
 *  that Baileys will auto-normalize into a native flow button. */
export type WAButtonShape = {
    /** native flow button name, e.g. 'quick_reply', 'cta_url', 'cta_call', 'cta_copy', 'single_select' */
    name: string;
...
type WithNativeFlowButtons = {
    /** Buttons to render as a native flow (button/native flow/bottom sheet) interactive message.
     *  Accepts native flow shape, or common legacy shapes (auto-normalized). */
    buttons?: WAButtonShape[];
    /** alias of `buttons`, kept for compatibility with bots migrating from templateButtons */
    templateButtons?: WAButtonShape[];
    /** alias of `buttons`, the most explicit/preferred name for native flow buttons */
    interactiveButtons?: WAButtonShape[];
    /** header title shown above the body text when buttons/sections are present */
    title?: string;
    /** footer text shown below the body text */
    footer?: string;
    /** list sections, renders as a `listMessage` instead of buttons */
    sections?: proto.Message.ListMessage.ISection[];
    /** button label that opens the list, used together with `sections` */
    buttonText?: string;
};
type WithDimensions = {
    width?: number;
...
export type AnyRegularMessageContent = (({
    ...
    body?: string;
    footer?: string;
} | SharePhoneNumber | RequestPhoneNumber) & ViewOnce & WithNativeFlowButtons;
