...
import { ExtendedMessageBuilder } from './dugong.js';
...
export const makeMessagesSocket = (config) => {
    ...
    const extendedMessages = new ExtendedMessageBuilder({
        generateWAMessageContent,
        generateMessageID,
        generateWAMessage,
        generateWAMessageFromContent,
        prepareWAMessageMedia
    }, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' &&
                ...
                const extendedType = extendedMessages.detectType(content);
                if (extendedType) {
                    switch (extendedType) {
                        case 'PAYMENT': {
                            const paymentContent = await extendedMessages.handlePayment(content, quoted);
                            return await relayMessage(jid, paymentContent, {
                                messageId: generateMessageID()
                            });
                        }
                        case 'PRODUCT': {
                            const productContent = await extendedMessages.handleProduct(content, jid, quoted);
                            const productMsg = await generateWAMessageFromContent(jid, productContent, { quoted });
                            return await relayMessage(jid, productMsg.message, {
                                messageId: productMsg.key.id,
                                additionalNodes: buildInteractiveAdditionalNodes(jid, productMsg.message)
                            });
                        }
                        case 'INTERACTIVE': {
                            const interactiveContent = await extendedMessages.handleInteractive(content, jid, quoted, options);
                            const interactiveMsg = await generateWAMessageFromContent(jid, interactiveContent, { quoted });
                            return await relayMessage(jid, interactiveMsg.message, {
                                messageId: interactiveMsg.key.id,
                                additionalNodes: buildInteractiveAdditionalNodes(jid, interactiveMsg.message)
                            });
                        }
                        case 'ALBUM':
                            return await extendedMessages.handleAlbum(content, jid, quoted);
                        case 'EVENT':
                            return await extendedMessages.handleEvent(content, jid, quoted);
                        case 'POLL_RESULT':
                            return await extendedMessages.handlePollResult(content, jid, quoted);
                        case 'GROUP_STORY':
                            return await extendedMessages.handleGroupStory(content, jid, quoted);
                    }
                }
                ...
                additionalNodes.push(...buildInteractiveAdditionalNodes(jid, fullMsg.message));
