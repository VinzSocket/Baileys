...
/** Normalizes a community-style buttons/templateButtons/interactiveButtons array into
 *  WhatsApp's native flow button shape: `{ name, buttonParamsJson }`. */
export declare const normalizeToNativeFlowButtons: (buttons: import("../Types/Message.js").WAButtonShape[]) => proto.Message.InteractiveMessage.NativeFlowMessage.INativeFlowButton[];
/** Wraps interactive/button/list/product message content in a viewOnceMessage with
 *  deviceListMetadataVersion set. See buildInteractiveAdditionalNodes for the binary
 *  node hint that's also required for the message to actually render. */
export declare const patchInteractiveMessage: (m: proto.IMessage) => proto.IMessage;
/** Builds the `<biz><interactive/></biz>` / `<biz><list/></biz>` (+ `<bot biz_bot="1"/>`
 *  for 1:1 chats) binary nodes required for interactive/button/list messages to render.
 *  Pass the result as `additionalNodes` to relayMessage(). */
export declare const buildInteractiveAdditionalNodes: (jid: string, m: proto.IMessage) => Array<{
    tag: string;
